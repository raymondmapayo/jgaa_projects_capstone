...
function DateFilterModal({ onApply }: DateFilterModalProps) {
  const today = dayjs();
      ...
      <Modal open={isModalOpen} footer={null} onCancel={handleCancel} centered>
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          <div className="border px-3 py-1 rounded-md">
            {startDate ? startDate.format("MMM DD") : "Start"}
          </div>
          <AiOutlineArrowRight className="text-xl text-gray-500" />
          <div className="border px-3 py-1 rounded-md">
            {endDate ? endDate.format("MMM DD") : "End"}
          </div>
        </div>
        ...
        <div className="flex justify-between gap-4">
          <div className="flex-1">
            ...
            <DatePicker
              value={startDate}
              onChange={(val: Dayjs | null) => setStartDate(val)}
              format="MMM DD, YYYY"
              style={{ width: "100%" }}
            />
          </div>
          ...
          <div className="flex-1">
            ...
            <DatePicker
              value={endDate}
              onChange={(val: Dayjs | null) => setEndDate(val)}
              format="MMM DD, YYYY"
              style={{ width: "100%" }}
            />
          </div>
