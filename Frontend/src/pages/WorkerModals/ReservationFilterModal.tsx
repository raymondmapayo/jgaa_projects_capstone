// src/components/admin/ReservationFilterModal.tsx
...
function ReservationFilterModal({
  onApply,
  onReset,
  ...
  const handleApply = () => {
    ...
    setDates([startDate, endDate]); // ✅ sync the displayed range
    setIsModalOpen(false);
  };

  const handleReset = () => {
    const defaultStart = firstDayOfMonth;
    const defaultEnd = today;
    setStartDate(defaultStart);
    setEndDate(defaultEnd);
    setDates([defaultStart, defaultEnd]);
    if (onReset) onReset();
    setIsModalOpen(false); // ✅ close modal after reset for better UX
  };
      ...
      <Modal open={isModalOpen} footer={null} onCancel={handleCancel} centered>
        ...
        <div className="flex items-center justify-center gap-3 mb-4">
          <div className="border px-3 py-1 rounded-md">
            {dates[0]?.format("MMM DD") ?? "Start"}
          </div>
          <AiOutlineArrowRight className="text-xl text-gray-500" />
          <div className="border px-3 py-1 rounded-md">
            {dates[1]?.format("MMM DD") ?? "End"}
          </div>
        </div>
        ...
        <div className="flex justify-between gap-4">
          <div className="flex-1">
            ...
            <DatePicker
              ...
              onChange={(val: Dayjs | null) => {
                setStartDate(val);
                setDates([val, endDate]); // ✅ keep display synced
              }}
              format="MMM DD, YYYY"
              style={{ width: "100%" }}
            />
          ...
          <div className="flex-1">
            ...
            <DatePicker
              ...
              onChange={(val: Dayjs | null) => {
                setEndDate(val);
                setDates([startDate, val]); // ✅ keep display synced
              }}
              format="MMM DD, YYYY"
              style={{ width: "100%" }}
            />
